...
import com.lightbend.kafkaclientmetrics.Domain
import com.lightbend.kafkaclientmetrics.PrometheusEndpoint.{GaugeDefinition, Message, Metric, MetricDefinitions}
...
object Metrics {
  ...
  val metricDefinitions: MetricDefinitions = Map(
    ...
    MaxGroupOffsetLagMetric.getClass -> GaugeDefinition(
      "kafka_consumergroup_group_max_lag",
      "Max group offset lag",
      "cluster_name", "group", "state", "is_simple_consumer"
    ),
    MaxGroupTimeLagMetric.getClass -> GaugeDefinition(
      "kafka_consumergroup_group_max_lag_seconds",
      "Max group time lag",
      "cluster_name", "topic", "partition"
    ),
    LastGroupOffsetMetric.getClass -> GaugeDefinition(
