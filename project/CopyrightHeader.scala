...
object CopyrightHeader extends AutoPlugin {
  ...
  val CopyrightPattern =
    "Copyright \\([Cc]\\) (\\d{4}(-\\d{4})?) Sean Glover <.*>".r
      ...
      def updateLightbendHeader(header: String): String = header match {
        case CopyrightHeaderPattern(years, null, _) =>
          if (years != CurrentYear)
            CopyrightPattern.replaceFirstIn(
              header,
              headerFor(years + "-" + CurrentYear)
            )
          else
  ...
  val cStyleComment = HeaderCommentStyle.cStyleBlockComment
    .copy(commentCreator =
      lightbendCommentCreator(
        HeaderCommentStyle.cStyleBlockComment.commentCreator
      )
    )
}
