...
trait LagSim extends KafkaSpec with ScalaFutures {
  ...
  class LagSimulator(topic: String, group: String) {
    ...
    private lazy val (consumerControl, consumerProbe) = Consumer
      ...
      .map { elem =>
        log.debug(
          "Committing elem with offset: {}",
          elem.committableOffset.partitionOffset
        )
  ...
  def lagSimActor(
      simulator: LagSimulator,
      scheduledTick: Cancellable = Cancellable.alreadyCancelled
  ): Behavior[Simulator] =
    Behaviors.receive[Simulator] {
      case (context, tick @ Tick(produce, consume)) =>
        ...
        lagSimActor(
          simulator,
          context.scheduleOnce(1 second, context.self, tick)
        )
